 ...
 *)
let initialize size =
  ( { grid = Array.make_matrix size size Empty; size }
  , { pos = [| size / 2, size / 2; size / 2, (size / 2) + 1 |]; dir = Top } )
;;
...
let rec bite (x, y) snake i =
  if i = Array.length snake.pos
  then false
  else (
    let a, b = snake.pos.(i) in
    if a = x && b = y then true else bite (x, y) snake (i + 1))
;;
...
let addApple snake world x y =
  if not (bite (x, y) snake 0) then world.grid.(x).(y) <- Full
;;
...
let displayApple i j =
  ...
  Graphics.fill_ellipse
    ((i * cellSize.(0)) + (cellSize.(0) / 2))
    ((j * cellSize.(1)) + (cellSize.(1) / 2))
    (cellSize.(0) / 2)
    (cellSize.(1) / 2);
...
let displayWorld world =
  for i = 0 to world.size - 1 do
    for j = 0 to world.size - 1 do
      if world.grid.(i).(j) = Full
      then (
        displayCell i j 0xffffff;
        displayApple i j)
      else displayCell i j 0xffffff
    done
  done
;;
...
let displaySnake snake =
  for i = 0 to Array.length snake.pos - 1 do
    ...
    if i = Array.length snake.pos - 1
    then displayCell x y 0xaaff44
    else displayCell x y 0x55ff55
  done
;;
...
let moveSnake (x, y) dx dy world snake =
  ...
  if nx < 0 || nx > world.size || ny < 0 || ny > world.size || bite (nx, ny) snake 0
  then raise Lose
  else if Array.length snake.pos >= 10
  then raise Win
  else nx, ny
;;
...
let run snake world =
  ...
  | Right ->
    ...
    if eatApple world n
    then Array.append snake.pos [| n |]
    else Array.sub (Array.append snake.pos [| n |]) 1 (Array.length snake.pos)
  | Left ->
    let n = moveSnake snake.pos.(Array.length snake.pos - 1) (-1) 0 world snake in
    if eatApple world n
    then Array.append snake.pos [| n |]
 ...
 *)
let move snake world = snake.pos <- run snake world
...
let program snake world fKey =
  while true do
    sleep 1.0;
    if Graphics.key_pressed () then fKey snake (Graphics.read_key ());
    move snake world;
    display snake world
  done
;;
